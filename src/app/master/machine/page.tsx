...
const EPage = () => {
  const [activeTab, setActiveTab] = useState("TUBE MILL");
  ...
  const fetchTabData = async (tabKey: string) => {
    ...
    try {
      ...
      if (response.msgId === 200) {
        ...
        if (tabKey === "TUBE MILL") {
          setTabData(response.data.Table8);
        }     
      ...
      console.error("Error fetching tube mill:", error);
...
const handleDelete = (record: any) => {
  Modal.confirm({
    ...
    onOk: async () => {
      try {
        ...
        const response = await apiClient(`${API_BASE_URL}DelM${activeTab == 'TUBE MILL' ? 'TubeMill' : activeTab}?PK_SRNO=${record.value}&USER_SRNO=${USER_SRNO}&UT_SRNO=${UT_SRNO}`,"DELETE");
  ...
  const handleSubmit = async (values: { name: string }) => {
    ...
    try {
      const response = await apiClient(`${API_BASE_URL}IuM${activeTab == 'TUBE MILL' ? 'TubeMill' : activeTab}`, "POST", payload); 
      if (response.msgId === 200) {
    ...
    <div>
      ...
      >
        <Form
          ...
          form={masterForm}
        >
          <Form.Item name="name" label="Tube Mill Name" rules={[{ required: true, message: "Please enter a value!" }]}>
            <Input placeholder="Enter Tube Mill Name" />
          </Form.Item>
          <Form.Item>
